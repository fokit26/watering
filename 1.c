...
unsigned char datar;
unsigned int adcH(void){
   ADCSRA|= (1<<ADSC);
   while(ADCSRA &(1<<ADSC)){}
   return ADCH; 
}
unsigned int adcL(void){
   ADCSRA|= (1<<ADSC);
   while(ADCSRA &(1<<ADSC)){}
   return ADCL; 
}
...
while (1)
      ...
      lcd_clear();
      datar = adcH();
      lcd_write_byte(0x27, datar/1000+0x30);
      lcd_write_byte(0x27, datar%1000/100+0x30);
      lcd_write_byte(0x27, datar%100/10+0x30);
      lcd_write_byte(0x27, datar%10+0x30);
      if(datar<200){
        PORTB=(0<<PORTB0);
      }
      if(datar>100){
